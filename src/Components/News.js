...
export class News extends Component {
  ...
  constructor(props) {
    ...
    document.title = `NewsGen - ${this.capitalizeFirstLetter(
      this.props.category
    )}`;
  }
  ...
  fetchMoreData = async () => {
    ...
    const url = `https://newsapi.org/v2/top-headlines?country=${this.props.country}&category=${this.props.category}&apiKey=5358deef8dbf46fc884374ca4d034c3f&page=${this.state.page+1}&pageSize=${this.props.pageSize}`;
    let data = await fetch(url);
    let parsedData = await data.json();
    this.setState({
  ...
  render() {
    return (
      <>
        ...
        {this.state.loading && <PulseLoader className='text-center' color="#36d7b7" />}
        <InfiniteScroll
